...
Once your program is running, the Pico 2 W will advertise itself over Bluetooth Low Energy (BLE) under the name **LED TrouBLE**.
...
When **LED TrouBLE** appears in the device list, connect to it and open the GATT table.
...
The workflow in nRF Connect for Mobile is the same: scan for **LED TrouBLE**, connect, expand service `0x180A`, and use the read, write, and notify controls on characteristic `0x2A57`.

### Troubleshooting

If **LED TrouBLE** does not show up in the scan list, check that the Pico is powered, your program is running, and Bluetooth is enabled on your laptop.
