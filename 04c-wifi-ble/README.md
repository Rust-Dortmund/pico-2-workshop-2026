...
> [!NOTE]
> `trouble-host` 0.8 is pulled from crates.io and needs `bt-hci` 0.10 (which depends on `bt-hci-transport` 0.1). Published `cyw43` 0.7.0 still talks the older `bt-hci` 0.8 API, so this crate patches the Embassy stack from a pinned [git revision](https://github.com/embassy-rs/embassy/commit/2d36ff0f8d7fbc3095412204f8f01dd2bcab6344) (see `[patch.crates-io]` in `Cargo.toml`). That `cyw43` implements `bt-hci-transport` and requires a few API updates in `main.rs` (dual DMA channels, `Cyw43439` chip type on `Runner`) - see the comments there. Current Embassy `main` is not usable yet: `embassy-net` renamed `proto-ipv4` to `ipv4`, which `picoserve` 0.20 still requires.
