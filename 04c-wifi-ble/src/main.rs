...
// BLE advertisement packets are limited to 31 bytes; our payload leaves 8 bytes for the name.
const _: () = assert!(BLE_NAME.as_bytes().len() <= 8);
...
async fn cyw43_task(
    ...
    runner: cyw43::Runner<'static, SpiBus<Output<'static>, PioSpi<'static, PIO0, 0>>, Cyw43439>,
) -> ! {
    runner.run().await
}
