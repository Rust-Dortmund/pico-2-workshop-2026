...
pub(crate) struct BleConnectionRunner {
    peripheral:
        Peripheral<'static, ExternalController<BleConnector<'static>, 20>, DefaultPacketPool>,
...
impl BleConnectionRunner {
    ...
    async fn handle_gatt_read<P: PacketPool>(event: ReadEvent<'_, '_, P>, server: &Server<'_>) {
        if event.handle() == server.led_service.color.handle {
            ...
            println!("[gatt] Read Event to Color Characteristic: {:?}", value);
        }
    ...
    async fn notify_task<P: PacketPool>(
        ...
        if let Some(new_color) = receiver.try_get()
            && server
                ...
                .is_err()
        {
            println!("[notify_task] error notifying connection");
            return;
        }
...
pub(crate) struct Ble {
    pub(crate) ble_runner:
        Runner<'static, ExternalController<BleConnector<'static>, 20>, DefaultPacketPool>,
    pub(crate) connection_runner: BleConnectionRunner,
}

pub(crate) fn initialize(
    esp_wifi_controller: &'static EspWifiController<'static>,
    ble_device: BT<'static>,
    sender: ColorSender<2>,
    receiver: ColorReceiver<2>,
) -> Ble {
    let ble_connector = BleConnector::new(esp_wifi_controller, ble_device);
