...
async fn main(_spawner: Spawner) {
    ...
    loop {
        // Wait until a gesture is detected...
        if detector.any_gesture_detected().await {
            // ...find the LED state we progress into and set the LED to that state.
            state = state.proceed();
            let (red_level, green_level, blue_level) = state.get_rgb_levels();
            output_red.set_level(red_level);
            output_green.set_level(green_level);
            output_blue.set_level(blue_level);

            // Wait for the gesture detection to stop to avoid accidental progressing the LED state.
